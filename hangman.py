...
class HumanPlayer(Player):
    """Class for player to take their turn
    ...
    def turn(self, state):
        ...
        Args:
            state (object): represents the current state of the game

        Returns:
            str: value typed by the user
            
        Side effect:
            prompt the user by name to guess a letter or solve the entire puzzle
            prints the board
        """
        print(state)
        result = input(f"{self.name}, guess a letter or type a word to solve \
the puzzle:")
        return result

class ComputerPlayer(Player):
    """Class for computer player to take their turn.

    Attributes:
        name (str): the player's name.
    """
    def __init__(self, name, list):
        """Initializes a ComputerPlayer method

        Args:
            name (str): name of the computer player
            list (list of str): vocabulary list
        """
        self.name = name
        self.vocab = list
    ...
    def turn(self, state):
        ...
        Returns:
            random unguessed letter or word to solve the puzzle
        """
        guess = choice(list(state.unguessed))
        guess_word = choice(self.vocab)
        if state.blank_count < 3:
            return guess
        else:
            return guess_word
...
class GameState:
    ...
    Attributes:
        board (str): a representation of the board, with unguessed letters
            represented as dots ("\u2022").
        expr (str): a regular expression that matches the current board. Used
            by the match() method.
        letter_count (int): number of letters in the word.
        guesses (set of str): characters that have already been guessed. All
            characters are upper-case.
        bad_guesses (set of str): already-guessed characters that are not in
            the word. All characters are upper-case.
        good_guesses (set of str): already-guessed characters that are in the
            word. All characters are upper-case.
        blank_count (int): number of unguessed characters in the word.
        unguessed (set of str): characters that have not been guessed yet.
        guessed_words (set of str): words that have been guessed. All words
    ...
    def __init__(self, word, guesses, guessed_words, score, max_score):
        ...
        self.expr = ("^"
                     ...
                     + "$")
        self.letter_count = len(re.findall(r"[A-Z]", word))
