...
class Cities:
    """ A class for finding the closest cities to a reference point
    
    Attributes:
        file (str): a path to a file containing city data
    """
    def __init__(self, file):
        """Initiates a Cities method

        Args:
            file (str): a path to a file containing city data
            
        Side effects:
            sets name, lat, lon, pop attributes
        """
        self.cities = []
    ...
    def nearest(self, lat, lon, min_population = 0, n = 10):
        """finds the closest city dictionaries from self.cities 
            with a population greater than or equal to the minimum population 
            specified in min_population

        Args:
            lat (float): a latitude
            lon (float): a longitude
            min_population (int, optional): a minimum population. Defaults to 0.
            n (int, optional): a maximum number of results to return. 
                Defaults to 10.

        Returns:
            list: a list of the n closest city dictionaries from self.cities 
            with a population greater than or equal to the minimum population 
            specified in min_population
        """
        cities = sorted(self.cities, 
                        key = lambda c: get_dist((lat,lon), 
                                                 (c["lat"],c["lon"])))
